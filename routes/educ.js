...
router.get('/educ', async (req, res) => {
  try {
    ...
    if (closestMatch) {
      ...
      const apiUrl = `https://hercai.onrender.com/v3/hercai?question=When I ask a serious question, You should respond seriously and provide a long answer over paragraphs. You also empasized the importance of including a source link from Google. your response should be in the language of the question and have you have specific mood while responding. and add a emoji to enchance the clarify of your respond: 📎source: (put the source in my question her.) 🌎 background information: (put the background information in my question here) 👩‍🏫 Fun Fact: (put the fun fact of my question here) 🔎 Search More (put the more information here in my question) make it by paragraph. here's my question:${encodeURIComponent(userQuestion)}`;
